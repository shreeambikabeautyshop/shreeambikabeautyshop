...
export default function CustomersPage() {
  ...
  const waMsg = (name: string) => encodeURIComponent(
    `Hi ${name}! This is Vinod from Shree Ambika Beauty Shop. `
  );
      ...
      <div className="flex gap-2 mb-6">
        <button onClick={() => setTab("customers")}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
            tab === "customers" ? "bg-brand-primary text-white shadow" : "bg-white text-gray-600 border border-gray-200 hover:border-brand-primary"
          }`}>
          <FiUsers size={14} /> Registered ({customers.length})
        </button>
        <button onClick={() => setTab("leads")}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all ${
            tab === "leads" ? "bg-orange-500 text-white shadow" : "bg-white text-gray-600 border border-gray-200 hover:border-orange-400"
          }`}>
        ...
        <div>
          ...
          {leads.length === 0 ? (
            <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center text-gray-400">
              ...
              <p className="text-xs mt-1">Leads appear here when visitors submit the engagement popup</p>
            </div>
          ) : (
              ...
              {leads.map(lead => (
                <div key={lead.id}
                  className={`bg-white rounded-2xl border p-4 flex items-start gap-4 ${
                    !lead.contacted ? "border-orange-200 shadow-sm" : "border-gray-100 opacity-70"
                  }`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {!lead.contacted && (
                        <span className="bg-orange-100 text-orange-700 text-[10px] font-bold px-2 py-0.5 rounded-full">New Lead</span>
                      )}
                      {lead.contacted && (
                        <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                          <FiCheck size={8} /> Contacted
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-gray-600 mt-0.5">+91 {lead.phone}</p>
                    {lead.beauty_tip && (
                      <p className="text-xs text-gray-400 mt-1 line-clamp-1">💡 {lead.beauty_tip.slice(0, 60)}...</p>
                    )}
                  ...
                  <a
                    href={`https://wa.me/91${lead.phone}?text=${encodeURIComponent(
                      `Hi ${lead.name}! This is Vinod from Shree Ambika Beauty Shop Mumbai. You visited our website — I want to personally help with your beauty needs! 😊`
                    )}`}
                    target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-1 bg-green-500 hover:bg-green-600 text-white text-xs font-bold px-3 py-2 rounded-xl transition-colors flex-shrink-0">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
              <p className="text-3xl font-bold text-green-600">
                {customers.filter(c => (Date.now() - new Date(c.created_at).getTime()) < 7 * 86400000).length}
              </p>
                      ...
                      return (
                        <tr key={c.id}
                          ...
                          <td className="px-4 py-3 text-center" onClick={e => e.stopPropagation()}>
                            <a href={`https://wa.me/91${c.phone}?text=${waMsg(c.full_name)}`}
                              target="_blank" rel="noopener noreferrer"
              ...
              <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden"
                ...
                <div className="p-5 space-y-3 max-h-[60vh] overflow-y-auto">
                  <div className="bg-brand-light rounded-2xl p-4 border border-brand-primary/20">
                    <p className="text-[10px] font-black text-brand-primary uppercase tracking-wide mb-2">📅 Registration</p>
                    {(() => { const dt = formatDateTime(selected.created_at); return (
                      <div className="space-y-1">
                        <div className="flex justify-between"><span className="text-xs text-gray-500">Day</span><span className="text-xs font-bold text-gray-800">{dt.day}</span></div>
                        <div className="flex justify-between"><span className="text-xs text-gray-500">Date</span><span className="text-xs font-bold text-gray-800">{dt.date}</span></div>
                        <div className="flex justify-between"><span className="text-xs text-gray-500">Time</span><span className="text-xs font-bold text-brand-primary">{dt.time}</span></div>
                        <div className="flex justify-between"><span className="text-xs text-gray-500">Ago</span><span className="text-xs text-gray-600">{timeAgo(selected.created_at)}</span></div>
                      </div>
                    ); })()}
                  </div>
                  ...
                  <div className="flex items-start gap-3 bg-gray-50 rounded-xl p-3">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-800 text-sm">{selected.address}</p>
                      <p className="text-xs text-gray-500">{[selected.city, selected.state, selected.pincode].filter(Boolean).join(", ")}</p>
                    </div>
                  </div>
                </div>
                <div className="px-5 pb-5 flex gap-2">
                  <a href={`https://wa.me/91${selected.phone}?text=${waMsg(selected.full_name)}`}
                    target="_blank" rel="noopener noreferrer"
                  ...
                  {selected.email && (
                    <a href={`mailto:${selected.email}`}
                      className="flex items-center justify-center bg-blue-500 hover:bg-blue-600 text-white font-bold py-3 px-4 rounded-2xl text-sm transition-colors">
                      <FiMail size={16} />
                    </a>
                  )}
                </div>
