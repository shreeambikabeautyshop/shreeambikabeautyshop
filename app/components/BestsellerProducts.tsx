...
export default function BestsellerProducts() {
  ...
  useEffect(() => {
    ...
    query.then(({ data }) => {
      ...
      setProducts(list);
      setPage(0);
      // Pick random product as default featured
      if (list.length > 0) {
  ...
  const selectFeatured = (p: Product) => {
    setFeatured(p);
    setFeaturedImg(0);
  };
    ...
    <section className="py-12 bg-white" aria-labelledby="bestseller-heading">
      <div className="max-w-[1400px] mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <Link href="/products"
            className="text-sm font-semibold text-brand-primary border border-brand-primary/30 px-4 py-1.5 rounded-full hover:bg-brand-light transition-colors whitespace-nowrap self-start sm:self-auto">
            View All Products →
          </Link>
        </div>
        ...
        <div className="grid lg:grid-cols-[1fr_380px] gap-6">
          ...
          <div className="rounded-3xl border border-gray-100 shadow-sm overflow-hidden bg-white">
            {loading || !featured ? (
              ...
              <div className="flex flex-col md:flex-row h-full">
                ...
                {/* Image column */}
                <div className="md:w-64 flex-shrink-0 bg-brand-light flex flex-col">
                  {/* Main image */}
                  <div className="relative flex-1 min-h-[260px]" style={{ aspectRatio: "1/1" }}>
                    ...
                    <div className="flex gap-2 p-3 overflow-x-auto">
                      {featured.images.slice(0, 4).map((img, i) => (
                        <button key={i} onClick={() => setFeaturedImg(i)}
                          className={`relative w-12 h-12 rounded-lg overflow-hidden border-2 flex-shrink-0 transition-all ${i === featuredImg ? "border-brand-primary" : "border-transparent opacity-60 hover:opacity-100"}`}>
                ...
                <div className="flex-1 p-6 flex flex-col justify-between">
                  <div>
                    <p className="text-[10px] font-black text-brand-primary uppercase tracking-widest mb-1">{featured.brand}</p>
                    ...
                    <div className="flex items-baseline gap-3 mb-4">
                      <span className="text-3xl font-black text-gray-900">₹{featured.price.toLocaleString("en-IN")}</span>
                    ...
                    {benefits.length > 0 && (
                      <div className="mb-4">
                        <p className="text-xs font-bold text-gray-600 mb-2">Key Benefits:</p>
                    ...
                    <div className="flex flex-wrap gap-3 mb-5">
                      {[
                        ...
                        <span key={b.label} className="flex items-center gap-1 text-[10px] text-green-700 font-semibold bg-green-50 px-2 py-1 rounded-full">
                          {b.icon} {b.label}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex gap-3 flex-wrap">
                    <a href={`https://wa.me/918291455297?text=${waMsg}`}
                      ...
                      className="flex-1 flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white font-bold py-3 rounded-2xl transition-all shadow-sm text-sm min-w-[140px]">
                      <FaWhatsapp size={16} /> Buy on WhatsApp
                    </a>
                    <Link href={`/products/${featured.slug || featured.id}`}
                      className="flex items-center justify-center gap-2 border-2 border-brand-primary text-brand-primary hover:bg-brand-primary hover:text-white font-bold py-3 px-5 rounded-2xl transition-all text-sm">
                      <FiEye size={14} /> Details
                    </Link>
                    <button
                      onClick={() => toggle({ id: featured.id, name: featured.name, slug: featured.slug, brand: featured.brand, price: featured.price, mrp: featured.mrp, images: featured.images, rating: featured.rating, category: featured.category })}
                      className="flex items-center justify-center w-12 h-12 rounded-2xl border-2 border-gray-200 hover:border-brand-primary transition-all">
                      {has(featured.id)
          ...
          {/* ── RIGHT: Bestseller List ── */}
          <div className="flex flex-col">
            {/* Red header bar */}
            <div className="flex items-center justify-between bg-brand-primary text-white px-4 py-3 rounded-t-2xl">
              <span className="font-bold text-sm tracking-wide">🔥 Bestseller Products</span>
              <div className="flex gap-1">
                <button onClick={() => setPage((p) => Math.max(0, p - 1))} disabled={page === 0}
                  className="w-7 h-7 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center disabled:opacity-40 transition-all">
            ...
            {/* List */}
            <div className="border border-gray-100 rounded-b-2xl overflow-hidden divide-y divide-gray-50 shadow-sm">
              {loading
                ? Array.from({ length: 6 }).map((_, i) => (
                    <div key={i} className="flex gap-3 p-3 animate-pulse bg-white">
                ...
                : visible.map((p, idx) => (
                    <button key={p.id} onClick={() => selectFeatured(p)}
                      className={`w-full flex gap-3 p-3 text-left transition-all hover:bg-brand-light group ${featured?.id === p.id ? "bg-brand-light border-l-4 border-brand-primary" : "bg-white"}`}>

                      {/* Rank + image */}
                      <div className="relative flex-shrink-0">
                        <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-gray-50">
                          {p.images?.[0] ? (
                            <Image src={p.images[0]} alt={p.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="64px" />
                          ) : (
